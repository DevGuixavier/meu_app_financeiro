...
export function TabBar({
  ...
  return (
    <nav className="borda-sutil fixed inset-x-0 bottom-0 mx-auto flex w-full max-w-[420px] gap-1 bg-surface p-2 md:max-w-3xl md:justify-center">
      {ABAS.map((aba) => (
        <button
          key={aba.valor}
          type="button"
          onClick={() => aoSelecionar(aba.valor)}
          className={`flex-1 rounded-full py-2.5 text-sm transition-colors md:flex-none md:px-10 ${
